...
async function getLines () {
  return new Promise(resolve => {
    exec(`git log --shortstat --pretty=format:"%at %aN"   < ${tty}`, {
      cwd: process.cwd(),
      maxBuffer: 5000 * 1024
    }, (err, stdout) => {
      if (err) {
...
async function revList (out) {
  ...
  // 复制 html 目录
  helper.copyDir(path.resolve(__dirname, '../html'), path.resolve(__dirname, '../html'), disFolder)
  helper.msg(`success generate html in folder: ${out}`, 'success')
}
